# Example Product Recommendation Management component

This is an example implementation of a [TM Forum Product Recommendation Management](https://www.tmforum.org/oda/directory/components-map/core-commerce-management/TMFC050) component.

This folder is the Helm Chart package which you distribute or host in a Helm Chart Repository. This README describes the functionality of the Product Recommendation Management component. The source code is available at [../source/ProductRecommendation-v4](/source/ProductRecommendation-v4/). The source file readme contains all the implementation documentation.
...
* The *mandatory* TMF680 Recommendation Management Open API v4.

It has the following dependent APIs:
* The *mandatory* dependency on TMF620 Product Catalog Management Open API v5 — used to validate `category.href` references on POST operations to ensure the referenced category exists.

### Management function

In its **management function** it implements:
* An *optional* metrics API supporting the open metrics standard (formerly the prometheus de-facto standard). This metrics endpoint provides business metrics about all the Create/Update/Delete events for all the Product Recommendation resources (Recommendation, etc.).
...
* A `productrecommendationmanagement` microservice that implements the TMF680 Recommendation Management Open API v4.
* A `productrecommendationmanagementinitialization` microservice that registers the metrics microservice as a listener for recommendation business events. This is deployed as a Kubernetes Job that runs once when the component is initialised.
...
```
helm install r1 .\productrecommendation-v4 -n components
```
...
r1-productrecommendationmanagement    Complete
```
...
| `api.image`           | akumartmf/recommendationapi:0.4                | The image for the implementation of the main API microservice           |
| `api.port`            | 8080                                           | The port on which the API microservice listens                          |
| `metrics.image`       | akumartmf/productrecommendationmanagementmetrics:0.2 | The image for the open metrics microservice                       |
