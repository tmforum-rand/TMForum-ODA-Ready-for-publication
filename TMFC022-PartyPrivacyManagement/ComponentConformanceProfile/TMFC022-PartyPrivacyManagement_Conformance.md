...
therefore be treated as **mandatory for conformance**, regardless of the `required: false` flag set on
that entry in the manifest — TMF669 is the canvas-identity API and is always mandatory when present under
`securityFunction`.

The specification also lists **TMF672 (User Role Permission Management API)** under `securityFunction`,
marked `required: false`. TMF672 is present-but-ignored for conformance purposes — its presence in the
security function does not promote it to mandatory.
...
```json
{
    ...
    "ctkconfig": {
        ...
        "productName": "REFERENCE EXAMPLE PARTY PRIVACY MANAGEMENT",
