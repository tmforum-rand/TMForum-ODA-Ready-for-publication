...
- **TMF649 (v4.0.0)** <!-- display name not found in apiIndex.json (no TMF649 entry at all) — fill in manually -->
  Swagger:
...
since it is the canvas-identity API. The component must ensure that this API is correctly implemented and
accessible, or alternatively ensure that a valid `canvasSystemRole` is configured.

**TMF672 (User Role Permission Management API)** is also present under the Security Function (both v4.0.0 and
v5.1.0 specification entries, both `required: false`), but is treated as present-but-ignored for conformance
purposes since nothing else in the spec marks it required.
...
```json
{
    ...
    "ctkconfig": {
        ...
        "productName": "REFERENCE EXAMPLE RESOURCE PERFORMANCE MANAGEMENT",
