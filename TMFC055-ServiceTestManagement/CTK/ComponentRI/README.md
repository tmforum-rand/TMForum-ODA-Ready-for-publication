# Example Service Test Management component (v5)

This is an example implementation of a [TM Forum Service Test Management](https://www.tmforum.org/oda/directory/components-map/production/TMFC055) component.

This folder is the Helm Chart package which you distribute or host in a Helm Chart Repository. This README describes the functionality of the Service Test Management component. The source code is available at [../source/ServiceTest-v5](/source/ServiceTest-v5/). The source file readme contains all the implementation documentation.
...
* The *mandatory* TMF653 Service Test Management Open API v4.2.

It has the following dependent APIs:
* The *mandatory* dependency on TMF638 Service Inventory Management Open API v5 — used to validate `relatedService.href` references on POST operations to ensure the referenced service instance exists.

### Management function

In its **management function** it implements:
* An *optional* metrics API supporting the open metrics standard (formerly the prometheus de-facto standard). This metrics endpoint provides business metrics about all the Create/Update/Delete events for all the Service Test resources (ServiceTest, ServiceTestSpecification, etc.).
...
* A `servicetestmanagement` microservice that implements the TMF653 Service Test Management Open API v4.2.
* A `servicetestmanagementinitialization` microservice that registers the metrics microservice as a listener for service test business events. This is deployed as a Kubernetes Job that runs once when the component is initialised.
...
```
helm install r1 .\servicetest-v5 -n components
```
...
r1-servicetestmanagement      Complete
```
...
| `api.image`           | akumartmf/servicetestapi:0.1                  | The image for the implementation of the main API microservice            |
| `api.port`            | 8080                                          | The port on which the API microservice listens                           |
| `metrics.image`       | akumartmf/servicetestmanagementmetrics:0.1    | The image for the open metrics microservice                              |
