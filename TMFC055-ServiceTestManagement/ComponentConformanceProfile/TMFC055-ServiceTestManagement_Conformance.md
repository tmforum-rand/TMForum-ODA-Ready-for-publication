...
manifest — TMF669 is the canvas-identity API and is treated as always mandatory whenever it appears in
`securityFunction`.

**TMF672 (User Role Permission Management API)** is also present under the Security Function, but is
**not** promoted to mandatory — it is marked `required: false` and nothing else in the specification
elevates it, so it is present-but-ignored for conformance purposes.
...
```json
{
    ...
    "ctkconfig": {
        ...
        "productName": "REFERENCE EXAMPLE SERVICE TEST MANAGEMENT",
