# TMFC046 – WorkforceManagement – v1.2.0
...
manifest — it is the canvas-identity API and is always required when listed under `securityFunction`.

**TMF672 (User Role Permission Management API)** is also present under the Security Function, marked
`required: false`. It is not promoted to mandatory for conformance purposes — it is present but ignored,
since nothing else in the specification marks it as required.
...
```json
{
    ...
    "ctkconfig": {
        ...
        "productName": "REFERENCE EXAMPLE WORKFORCE MANAGEMENT",
