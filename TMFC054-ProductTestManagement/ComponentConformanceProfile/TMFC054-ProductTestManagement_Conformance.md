# TMFC054 – ProductTestManagement – v1.1.0

## Mandatory Exposed APIs (Require Conformance)

- **TMF769 – v5.0.0** <!-- display name not found in apiIndex.json or Ready-for-publication — fill in manually -->
  Swagger:
  - https://tmf-open-api-table-documents.s3.eu-west-1.amazonaws.com/Beta/TMF769_Product_Test/5.0.0/swagger/TMF769-Product_Test-v5.0.0.oas.yaml
...
manifest — TMF669 is the canvas-identity API and is always treated as mandatory when it appears under
`securityFunction`.

**TMF672 (User Role Permission Management API)** is also present under `securityFunction` (also marked
`required: false`), but it is not promoted to mandatory for conformance purposes — it is present-but-ignored
unless something else in the spec explicitly marks it required, which is not the case here.
...
```json
{
    ...
    "ctkconfig": {
        ...
        "productName": "REFERENCE EXAMPLE PRODUCT TEST MANAGEMENT",
