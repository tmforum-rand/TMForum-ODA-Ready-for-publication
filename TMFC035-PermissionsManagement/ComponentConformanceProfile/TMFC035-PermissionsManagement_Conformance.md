...
therefore be treated as **mandatory for conformance**, regardless of the `required: false` flag attached
to it in the manifest. **TMF672 (User Role Permission Management API)** is also listed under
`securityFunction`, but is present-but-ignored for security conformance purposes — it carries
`required: false` there and nothing else in the Security Function promotes it to mandatory. (Note: TMF672
is separately mandatory as a **core exposed API** per `coreFunction.exposedAPIs` above — that requirement
is independent of, and not derived from, its appearance in `securityFunction`.)
...
```json
{
    ...
    "ctkconfig": {
        ...
        "productName": "REFERENCE EXAMPLE PERMISSIONS MANAGEMENT",
