# Example Service Inventory Management component (v5)

This is an example implementation of a [TM Forum Service Inventory Management](https://www.tmforum.org/oda/directory/components-map/production/TMFC008) component.

This folder is the Helm Chart package which you distribute or host in a Helm Chart Repository. This README describes the functionality of the Service Inventory Management component. The source code is available at [../source/ServiceInventory-v5](/source/ServiceInventory-v5/). The source file readme contains all the implementation documentation.
...
* The *mandatory* TMF638 Service Inventory Management Open API v5.
...
* An *optional* metrics API supporting the open metrics standard (formerly the prometheus de-facto standard). This metrics endpoint provides business metrics about all the Create/Update/Delete events for all the Service Inventory resources (Service, etc.).
...
* A `serviceinventory` microservice that implements the TMF638 Service Inventory Management Open API v5.
* A `serviceinventoryinitialization` microservice that registers the metrics microservice as a listener for service inventory business events. This is deployed as a Kubernetes Job that runs once when the component is initialised.
...
```
helm install r1 .\serviceinventory-v5 -n components
```
...
| `api.image`           | akumartmf/serviceinventoryapiv5:0.1       | The image for the implementation of the main API microservice            |
| `api.port`            | 8080                                      | The port on which the API microservice listens                           |
| `metrics.image`       | akumartmf/serviceinventorymetricsv5:0.1   | The image for the open metrics microservice                              |
