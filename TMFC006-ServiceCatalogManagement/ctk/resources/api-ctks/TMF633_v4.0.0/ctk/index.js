...
var pmCollection = require('./TMF633-ServiceCatalog-v4.0.0.testkit.json');
...
Object.keys(config['payloads']).forEach(resource => {
    ...
    if (resource == "ServiceSpecification") {
...
function extractSslParameters(config){
    let allowedKeys =  [
        "sslClientCert",
        "sslClientKey",
        "sslClientPassphrase",
    ...
    if (config.sslconfig && Object.keys(config.sslconfig).length <= allowedKeys.length) {
        Object.keys(config.sslconfig).forEach(function(key){
            if (allowedKeys.indexOf(key) == -1) {
                console.log("ERROR: " + key + " is not a valid SSL parameter")
                process.exit(0)
            }
        })
...
function exportEnvironment(url) {
    ...
    envJson.values.forEach(element => {
        if (element.key == "Service_Catalog") {
...
function runNewman() {
    ...
    console.log("Running with parameters", extractSslParameters(config))
    newman.run({
        collection: pmCollection,
        environment: require('./TMFENV.json'),
        insecure: true,
        reporters: ['html', 'json'],
        ...extractSslParameters(config),
        reporter: {
            html: {
