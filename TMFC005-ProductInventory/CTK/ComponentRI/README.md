# Example Product Inventory component (v5)

This is an example implementation of a [TM Forum Product Inventory Management](https://www.tmforum.org/oda/directory/components-map/core-commerce-management/TMFC005) component.

This folder is the Helm Chart package which you distribute or host in a Helm Chart Repository. This README describes the functionality of the Product Inventory component. The source code is available at [../source/ProductInventory-v5](/source/ProductInventory-v5/). The source file readme contains all the implementation documentation.
...
* The *mandatory* TMF637 Product Inventory Management Open API v5.

It has the following dependent APIs:
* The *mandatory* dependency on TMF620 Product Catalog Management Open API v5 — used to validate `productOffering.href` references on POST operations to ensure the referenced product offering exists.
* The *optional* dependency on TMF639 Resource Inventory Management Open API v4.
* The *optional* dependency on TMF638 Service Inventory Management Open API v4.

### Management function

In its **management function** it implements:
* An *optional* metrics API supporting the open metrics standard (formerly the prometheus de-facto standard). This metrics endpoint provides business metrics about all the Create/Update/Delete events for all the Product Inventory resources (ProductInstance, etc.).
...
* A `productinventory` microservice that implements the TMF637 Product Inventory Management Open API v5 (listening on port 8637).
* A `productinventoryinitialization` microservice that registers the metrics microservice as a listener for product inventory business events. This is deployed as a Kubernetes Job that runs once when the component is initialised.
...
```
helm install r1 .\productinventory-v5 -n components
```
...
| `api.image`           | akumartmf/productinventoryapi:0.1    | The image for the implementation of the main API microservice              |
| `api.port`            | 8637                                 | The port on which the API microservice listens                             |
| `metrics.image`       | akumartmf/productinventorymetrics:0.1 | The image for the open metrics microservice                               |
