    ...
    it('CTKs for exposed apis have been executed successfully', async function () {
        ...
        for (api of status_apis) {
            ...
            for (body_api of body_apis) {
                if (body_api.implementation === impl) {
                    let spec = body_api.specification?.[0]?.url || ""
                    ...
                    // Extract all occurrences of "TMF" followed by numbers
                    console.log(`spec value is: ${spec}`)
                    const matches = spec.match(regex);
                    console.log(`matches value is: ${ matches}`)
                    let api_ref = matches[0] + "_v" + swagger.info.version.split('.')[0]
                    console.log(`api_ref is ${api_ref} url: ${url}`)
                    //
        ...
        functions.forEach(f => {
            edges.forEach(e => {
                ...
                console.log(`gc_apis value is: ${gc_apis}`)
                ...
                gc_apis.filter(api => api.required || includeOptional).forEach(api => {
//                gc_apis.filter(api => api.required).forEach(api => {
                    console.log(`Configuring CTK for ${api.name}`)
                    
                    let apiVersion = api.specification?.[0]?.version || "unknown_version"; 
                    let majorVersion = apiVersion.split('.')[0];  // Extract major version
                    api_ref = api.id + "_" + majorVersion
                    console.log(`api_ref for ${api.name} is ${api_ref}`)
                    ctk_location = Path.join("../resources/api-ctks", api_ref)
                    if (!fs.existsSync(ctk_location)) {
                        if (!api.required) {
                            console.log(`Skipping CTK for ${api_ref} as it is optional and CTK is not found.`)
                            return
                        }
                    }
                    console.log(`ctk_location for ${api_ref} will be: ${ctk_location}`)
                    if (f === "securityFunction" && e === "exposedAPIs" && !ref_to_url[api_ref]) {
                        console.log(`Skipping CTK for security function API ${api_ref} as implementation not found and is optional`)
                    } else {
                        api_configs.push({
                            path: ctk_location,
        ...
        let configured_ctks = api_configs.map(async api => {
            try {
                ...
                console.log("Configuring config.json", ctk_config.url, ctk_config.headers)
...
function runAPICTK(apiData) {
    ...
    console.log(`Running api CTK for ${apiData.api_ref}`)
