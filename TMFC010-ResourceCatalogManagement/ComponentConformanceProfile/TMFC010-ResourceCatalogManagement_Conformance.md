# TMFC010 – ResourceCatalogManagement – v1.4.2
...
manifest — it is the canvas-identity API. **TMF672 (User Role Permission Management API)** is also present
under `securityFunction` (also marked `required: false`), but is treated as present-but-ignored for
conformance purposes: nothing elsewhere in the specification promotes it to mandatory.
...
```json
{
    ...
    "ctkconfig": {
        ...
        "productName": "REFERENCE EXAMPLE RESOURCE CATALOG MANAGEMENT",
