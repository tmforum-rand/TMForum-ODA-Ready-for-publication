# TMFC020 – DigitalIdentityManagement – v1.2.0

## Mandatory Exposed APIs (Require Conformance)

- **TMF720 – Digital Identity Management API**
  Swagger:
  - https://tmf-open-api-table-documents.s3.eu-west-1.amazonaws.com/OpenApiTable/TMF720_Digital_Identity/4.0.0/swagger/TMF720_Digital_Identity_Management_API_v4.0.0_swagger.json
...
therefore be treated as **mandatory for conformance**. Note that TMF669 is marked `required: false` within
`securityFunction.exposedAPIs`, but per the standard rule it is treated as mandatory regardless of that flag
whenever it appears in the Security Function — and here it is independently reinforced as mandatory, since
the same API also appears in `coreFunction.dependentAPIs` with `required: true`.

Also present under the Security Function is **TMF672 (User Role Permission Management API)**, listed with
`required: false`. TMF672 is present-but-ignored for conformance purposes — it is not promoted to mandatory
solely by virtue of sitting inside `securityFunction`.
...
```json
{
    ...
    "ctkconfig": {
        ...
        "productName": "REFERENCE EXAMPLE DIGITAL IDENTITY MANAGEMENT",
