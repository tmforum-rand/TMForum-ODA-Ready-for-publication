...
this flag is overridden for TMF669 because it is the canvas-identity API. **TMF672 (User Role Permission
Management API)** is also present under `securityFunction` (also `required: false`), but is treated as
present-but-ignored for conformance purposes; it is not promoted to mandatory.
...
```json
{
    ...
    "ctkconfig": {
        ...
        "productName": "REFERENCE EXAMPLE RESOURCE ORDER MANAGEMENT",
